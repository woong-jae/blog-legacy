...
문자형이나 심볼형에 속하지 않은 값은 문자열로 자동변환된다.
...
// -> 공부하는중...
```
`[student]`는 속성의 이름을 바인딩 `student`에서 가져오겠다는 것을 의미한다.
...
따라서, 값 자체를 변경하는게 아닌 **새로운 값을 가리키게** 되는 것이다. `String.slice()` 역시 문자열을 변경하는 것이 아니라 새로운 문자열을 반환하는 것이다.
