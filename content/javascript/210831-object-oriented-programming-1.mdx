...
자바스크립트에서 constructor를 더 쉽게 정의할 수 있는 방법을 제공한다. **함수 호출 부분 앞에 `new` 키워드를 사용하면 그 함수는 constructor로 작용**한다.
...
`weirdRabbit`의 프로토타입이 `Rabbit.prototype`이다. Constructor의 **'prototype' 속성 안에 인스턴스의 프로토타입**이 있다.
