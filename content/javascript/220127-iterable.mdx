...
1번 방식과 2번 방식의 차이점에는 '관심사의 분리(Seperate of Concern, SoC)'다.
...
2번 방식은 `range[Symbol.iterator]()`가 대상 객체를 반환한다. 반환된 객체에는 `next()`도 있고 `this.current`에 반복이 얼마나 진행되었는지 나타내는 값도 저장되어 있다. `range` 자체가 itertor가 된 것이다.
...
- **유사 배열(Array-like)**는 인덱스와 `length` 속성이 있어 배열처럼 보이는 객체다.
...
`Array.from`은 `str.split`과 다르게 문자열 자체가 가진 iterable 속성을 이용해 동작한다. 따라서 **`for..of`처럼 서로게이트 쌍에도 제대로 적용**된다.
```js
let str = '𝒳😂';
