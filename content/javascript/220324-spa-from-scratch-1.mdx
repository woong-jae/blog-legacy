...
이전에 React를 사용해봤었기 때문에 또 React를 사용하면 수월하게 개발할 수 있었다.
하지만, 프로젝트 크기가 작아서 React로 개발해봤자 뭔가 새로 배울게 없을 것 같았다.
...
"어떻게든 활용할 수 없을까"해서 이러저리 2주 정도 만져보다가, "아직 이 녀석은 이르다"라는 판단을 하고 접게 되었다.
...
따라서 원래 `constructor`의 역할을 떼어내서 `setup`이라는 메서드를 새로 만들어주자.
...
이 정도면 내가 원하던 대로 컴포넌트 단위 개발이 가능할 것 같아 개발을 시작했다.
