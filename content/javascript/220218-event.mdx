...
브라우저는 **단일 쓰레드에서 event-driven** 방식으로 동작한다. 쓰레드가 하나뿐이라는 의미는 하나의 작업만을 처리할 수 있다는 것을 의미하지만,
이벤트 루프 덕분에 여러 작업이 동시에 처리되는 것처럼 느껴진다.
...
- **Event queue(task queue)*: 비동기 처리 함수의 콜백, 비동기식 이벤트 핸들러, Timer(`setTimeout`, `setInterval`) 함수의 콜백이 보관되는 영역이다.
...
태스크는 크게 두가지로 나눌 수 있다. ***매크로태스크(macrotask)***와 ***마이크로태스크(microtask)***다.

태스크는 자바스크립트 엔진이 바쁠 때 추가될 수도 있는데, 이때 새로운 태스크는 큐에 '매크로태스크 큐(= 콜백 큐, 태스크 큐)'에 추가된다.
`setTimeout`, `setInterval`, `setImmediate`, `requestAnimationFrame`, I/O, UI 렌더링 등이 이에 해당된다.

'마이크로태스크 큐'(PromiseJobs 큐)에는 프로미스의 콜백함수가 들어간다.

**자바스크립트 엔진은 매크로태스크 하나를 처리할 때마다 또 다른 매크로태스크나 렌더링 작업을 하기 전에 마이크로태스크 큐에 쌓인 마이크로태스크를 전부 처리한다.**
마이크로태스크는 다른 이벤트 핸들러나 렌더링 작업, 혹은 다른 매크로태스크가 실행되기 전에 전부 처리된다.
...
위 코드는 `code`, `promise`, `timeout` 순서대로 출력된다.
...
이벤트가 전파되는 방향에 따라 ***버블링(event bubbling)***과 ***캡처링(event capturing)***으로 구분할 수 있다.
