...
블록 내 바인딩의 가시성은 그 블럭의 위치에 의해 결정된다. 각 지역 스코프는 해당 범위를 포함하는 모든 지역 스코프를 볼 수 있고, 모든 스코프는 전역 스코프를 볼 수 있다.
바인딩 가시성에 대한 이런 접근 방식을 'lexical scoping'이라고 한다. '내가 사용할 변수가 어디서 사용가능'한지 알기 위해 **그 변수가 어디서 선언**되었는지 고려한다.
이것을 통해 함수내에서 외부 범위의 변수를 참조가 가능해진다.
...
함수가 만들어졌을 때 그 환경을 기억하는 기능이다. 클로저 덕분에 바인딩의 수명(lifetime)에 대한 걱정을 덜 수 있게 된다. 이런 식의 코드 작성도 가능해진다.
```js
