...
---
## SVM Classifier
SVM classifier는 두 개의 그룹을 분류할 때, **두 그룹간 margin이 최대가 되게 하는 hyperplane**을 찾는 것이 목표다.
...
$d(x)$가 0보다 큰 것과 작은 것, 두 공간으로 나눌 수 있다.

SVM의 목표는 **margin이 가장 큰 hyperplane의 방항 $w$를 찾는 것**이다.
...
## SVM regression
SVM regression은 특정 margin 안에 인스턴스가 최대한 많이 들어오게 하는 것이 목표다. 
