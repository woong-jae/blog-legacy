...
데이터 시각화에도 아주 유용하다. 고차원 훈련 세트의 차원 수를 둘로(또는 셋) 줄이면 하나의 압축된 그래프를 그릴 수 있게 된다.
...
트레이닝 인스턴스가 일률적으로 퍼져있지 않고 특정 영역이 비어있다고 가정했을 때, 더 낮은 차원에 모든 훈련 샘플을 투영하는 방법이다.  

단점: '어느 평면에 projection 할 것인가'가 문제다. 그리고 데이터셋의 부분 공간이 뒤틀리거나 휘어져있으면 효과적이지 않다.

#### Manifold Learning
내제된 manifold가 있을 것이라고 가정하는 방법이다. 

훈련 샘플이 놓여 있는 매니폴드를 모델링한다. 매니폴드 가정에 의해 고차원인 실제 데이터 셋이 더 낮은 저차원 매니폴드에 가깝게 놓여있다고 가정한다.

## PCA(Principle Component Analysis)
PCA(주성분 분석)는 데이터에 가장 가까운 초평면을 구한 다음, 데이터를 이 초평면에 투영시키는 방법이다. PCA는 두 단계로 나뉜다.
...
직관적으로 데이터가 잘 분산되어 있어야 정보가 적게 손실되었다고 판단할 수 있다.

데이터를 잘 구분할 수 있을 때, 즉, 데이터가 가장 퍼지는 hyperplane에 투영한 것이라고 할 수 있다. 
데이터를 가장 퍼지게 할라면 투영된 훈련 샘플들의 **분산이 가장 큰 것**을 찾으면 된다.

다르게 말하면 원본 데이터셋과 투영된 것 사이의 평균 제곱 거리를 최소화화는 축이다.

1. Variance가 가장 큰 축을 선택한다.
2. 선택한 축에 직교하면서 분산이 최대인 축을 찾는다.  
3. 첫 번째 축과 두 번째 축에 직교하고 분산이 최대인 세 번째 축을 찾는다.
...
투영에 의존하지 않고 매니폴드 학습을 기반으로 둔다.

데이터 샘플을 기준으로 거리상 가까운 이웃이 있을 것이고 이웃들이 몰려있는 그룹이 여러개 있을 것이라고 가정했을 때, LLE는 인접한 이웃들과의 거리를 보존하면서 데이터셋을 저차원으로 축소한다.
...
- Kernel PCA: 커널 트릭을 사용한다.  
    문제점: 커널 PCA는 메모리 기반 방법이다. 훈련집합 X의 크기가 클 경우, 메모리 부담이 증대된다.
