...
1. **Primary Storage**: CPU에 의해 동작한다. Volatile, 즉, 전원이 꺼지면 사라지지만 빠르다.
...
Block(or page)는 여러 sector의 그룹이다(보통 4KB).
...
- Prefetching: 데이터를 요청 전에 읽기

- I/O 요청의 적절한 스케쥴링

- Log disk를 활용해서 쓰기 작업을 일시적으로 들고있기: seek time을 없애기 위해 쓰여야 할 block은 하나의 log disk로 간다.
...
- Fixed-length records: 파일은 모든 레코드의 크기가 같다. 구현과 관리가 쉽다. 하지만 공강 낭비가 발생할 수 있다.
...
Disk block을 **bucket**이라고 한다. Bucket은 하나이상의 레코드를 가지는 저장소의 단위가 된다.

**Hash field에 대한 quality condition에 한해서 굉장히 빠른 접근 속도를 제공**한다.