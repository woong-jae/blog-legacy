...
2. **Entity 무결성** 제약조건: 기본 키(PK) 속성은 `NULL`이 되면 안된다.

3. **Referential 무결정** 제약조건: 외래 키(FK)의 값은 참조하는 relation의 기본 키(PK)이어야 한다. `NULL`일 수도 있다.
...
**Super Key?**:  
SK를 어트리뷰트의 부분집합이라고 하면, 어떤 두 튜플도 같은 SK 값을 가지지 않아야 한다. ($t_1[SK]≠t_2[SK]$). 각 튜플을 유일하게 식별할 수 있는 하나 이상의 속성들의 조합이다.

**Key?**:  
'minimal'한 슈퍼 키다. 즉, 어떤 슈퍼 키에서 어느 속성 하나라도 제거했을 때 슈퍼 키가 아니게 된다면 Key다.  
**Key는 superkey이지만, 그 반대는 아니다**.

Primary Key(기본 키)는 relation에서 각 튜플들을 고유하게 식별해주는 역할을 한다. 또한, 튜플에서 다른 튜플을 참조할 때 사용한다. 이때 참조되는 키를 'foreign key(외래 키)'라고 한다.
